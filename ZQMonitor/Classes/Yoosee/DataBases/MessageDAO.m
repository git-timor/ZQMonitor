...
-(BOOL)insert:(P2PMessage*)message{
...
-(NSMutableArray*)findAllWithId:(NSString *)contactId{
    ...
    if([self openDB]){
        ...
        if(sqlite3_prepare_v2(self.db, [SQL UTF8String], -1, &statement, NULL)==SQLITE_OK){
            while(sqlite3_step(statement)==SQLITE_ROW){
                P2PMessage *data = [[P2PMessage alloc] init];
    ...
    NSComparator compareByTime = ^(id obj1,id obj2){
        P2PMessage *message1 = (P2PMessage*)obj1;
        P2PMessage *message2 = (P2PMessage*)obj2;
...
-(BOOL)delete:(P2PMessage*)message{
    sqlite3_stmt *statement;
    BOOL result = YES;
    if([self openDB]){
