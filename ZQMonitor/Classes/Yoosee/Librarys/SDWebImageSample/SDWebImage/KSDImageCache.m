...
static KSDImageCache *instance;

@implementation KSDImageCache
...
+ (KSDImageCache *)sharedImageCache
{
    if (instance == nil)
    {
        instance = [[KSDImageCache alloc] init];
    }
...
- (void)notifyDelegate:(NSDictionary *)arguments
{
    ...
    id <KSDImageCacheDelegate> delegate = [arguments objectForKey:@"delegate"];
...
- (void)queryDiskCacheForKey:(NSString *)key delegate:(id <KSDImageCacheDelegate>)delegate userInfo:(NSDictionary *)info
{
    if (!delegate)
    {
        return;
