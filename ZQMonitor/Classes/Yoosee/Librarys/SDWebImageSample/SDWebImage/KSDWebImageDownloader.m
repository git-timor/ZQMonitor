...
NSString *const KSDWebImageDownloadStartNotification = @"KSDWebImageDownloadStartNotification";
NSString *const KSDWebImageDownloadStopNotification = @"KSDWebImageDownloadStopNotification";

@interface KSDWebImageDownloader ()
@property (nonatomic, retain) NSURLConnection *connection;
@end

@implementation KSDWebImageDownloader
    ...
    if (NSClassFromString(@"SDNetworkActivityIndicator"))
    {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:activityIndicator
                                                 selector:NSSelectorFromString(@"startActivity")
                                                     name:KSDWebImageDownloadStartNotification object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:activityIndicator
                                                 selector:NSSelectorFromString(@"stopActivity")
                                                     name:KSDWebImageDownloadStopNotification object:nil];
    }

    KSDWebImageDownloader *downloader = [[[KSDWebImageDownloader alloc] init] autorelease];
    downloader.url = url;
...
- (void)start
{
    ...
    if (connection)
    {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:KSDWebImageDownloadStartNotification object:nil];
    }
    else
    {
...
- (void)cancel
{
    if (connection)
    {
        [connection cancel];
        self.connection = nil;
        [[NSNotificationCenter defaultCenter] postNotificationName:KSDWebImageDownloadStopNotification object:nil];
    }
}
